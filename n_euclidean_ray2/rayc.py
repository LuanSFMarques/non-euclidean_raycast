...
class RayCasting:
    ...
    def ray_cast(self):
        ...
        for ray in range(NUM_RAYS): #num of rays
            ...
            try:
                if final_final in self.game.map.world_map:
                    if self.game.map.world_map[final_final] in [1,2]:
                            x, y = final_final
                            pg.draw.rect(self.game.screen,C_TILE2,(((WIDTH//3)*2) + x * PIECES_SIZE,
                                                    y * PIECES_SIZE,
                                                    PIECES_SIZE,
                                                    PIECES_SIZE))
            except:
            ...
            # Projection
            wall_height = 21000 / (depth3d + 0.0001)
            ...
            try:
                if self.game.map.world_map[final_final] == 1:
                    wall_color = [161 / (1 + depth3d ** 5 * 0.00000000000002), 151 / (1 + depth3d ** 5 * 0.00000000000002),191 / (1 + depth3d ** 5 * 0.00000000000002)]
                else:
                    wall_color = [215 / (1 + depth3d ** 5 * 0.00000000000002), 202 / (1 + depth3d ** 5 * 0.00000000000002),255 / (1 + depth3d ** 5 * 0.00000000000002)]
                line_color = 'white'
            except:
                try:
                    if color_clone:
            ...
            # Draw Lines 2d
            pg.draw.line(self.game.screen,line_color,(ox1,oy1),(ox1 + depth * math.cos(ray_angle), oy1 + depth * math.sin(ray_angle)),3)
            pg.draw.line(self.game.screen, 'white', (ox1+HEIGHT_2, oy1), (ox1+HEIGHT_2 + depth * math.cos(ray_angle), oy1 + depth * math.sin(ray_angle)),3)

            # Draw Lines 2d CLONE
            if clone1:
                cx1, cy1 = ox1 - (PIECES_SIZE*7), oy1 - (PIECES_SIZE*2)

                pg.draw.line(self.game.screen,'white',(cx1,cy1),(cx1 + depth * math.cos(ray_angle), cy1 + depth * math.sin(ray_angle)),3)
                pg.draw.circle(self.game.screen, '#FFB36B', (cx1, cy1), 8)
            ...
            pg.draw.circle(self.game.screen, '#FFB36B', (ox1, oy1), 8)

            pg.draw.rect(self.game.screen, wall_color, (HEIGHT + ray * SCALE, (HEIGHT/2) - wall_height/2, SCALE, wall_height))
